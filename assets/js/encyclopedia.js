...
class EncyclopediaHandlerClass extends elementorModules.frontend.handlers.Base {
  ...
  showTerm(e) {
    ...
    this.elements.$currentDef.fadeOut('fast', () => {
      this.elements.$currentDef = this.$element.find(".encyclopedia__definition--".concat(this.elements.$currentTerm.data('index')));
      this.elements.$currentDef.fadeIn('fast');
    });

    if (breakpoint.matches) {
      this.$element.find('.encyclopedia__col--2').get(0).scrollIntoView({
        behavior: 'smooth'
      });
    }
  }
...
jQuery(window).on('elementor/frontend/init', () => {
  ...
  elementorFrontend.hooks.addAction('frontend/element_ready/encyclopedia.default', addHandler);
});